...
{
    public class OpenShiftKubernetesProvider : IKubernetesProvider
    {
        private readonly Kubernetes _client;
        private readonly ConfigurationService _configService;
        private readonly ILogger _logger;

        public OpenShiftKubernetesProvider(string kubeConfigPath, ConfigurationService configService, ILogger logger = null)
        {
        ...
        public async Task CreateNamespaceIfNotExistsAsync(string namespaceName)
        {
            ...
            {
                // For OpenShift, we create a Project instead of a namespace
                var namespaceObj = new V1Namespace
                {
                    Metadata = new V1ObjectMeta
                    {
                        ...
                        {
                            { "openshift.io/description", "Test Execution Platform namespace" },
                            { "openshift.io/display-name", namespaceName }
                        }
                    }
                };

                await _client.CreateNamespaceAsync(namespaceObj);
                _logger?.LogInformation($"Created namespace/project: {namespaceName}");
            }
        }
        ...
        public async Task<string> CreateTestJobAsync(string imageName, string jobName, string repoUrl, string namespaceParam)
        {
            ...
            var job = new V1Job
            {
                Metadata = new V1ObjectMeta
                {
                    ...
                    Annotations = new Dictionary<string, string>
                    {
                        { "openshift.io/description", "Test execution job" }
                    }
                },
                Spec = new V1JobSpec
                {
